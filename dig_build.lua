...
function bot_build(pos, buildy, filter, mode)
    ...
    if not minetest.is_protected(buildpos, bot_owner) then
        if buildnode.name == "air" then
            local found = nil
            -- prefer the selected block type from the program slot
            if filter ~= "" and minetest.registered_nodes[filter] then
                found = filter
            else
                -- fall back to first non-empty node item in inventory
                local content = inv:get_list("main")
                local a = 1
                if content then
                    while( a<33 and not found) do
                        if content[a] and not content[a]:is_empty() then
                            local nm = content[a]:get_name()
                            if minetest.registered_nodes[nm] then
                                found = nm
                            end -- if item is a registered node
                        end -- if stack non-empty
                        a=a+1
                    end -- loop over inventory slots
                end
            end -- if selected block else fallback
            if found then

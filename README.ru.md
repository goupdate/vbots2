...
| ![turn_danger](textures/vbots_turn_danger.png) | **Поворот→** | Повернуться к атакующему (последние 3с) или ближайшему hostile (сфера 30 блоков). После неё можно поставить **×N** (кнопка числа) — радиус поиска угрозы = N×30. Поворот только если цель найдена — иначе искры, без поворота. |
